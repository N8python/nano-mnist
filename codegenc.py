...
def generate_first_layer_code(used_neurons, used_inputs):
    ...
    if valid_neurons:
        declarations += "    int " + ", ".join([f"a{neuron_idx}" for neuron_idx in valid_neurons]) + ";\n"
    ...
    for neuron_idx in used_neurons:
        ...
        code += f"    a{neuron_idx} = relu({accumulator_expr}) ;\n"
        #code += f"    a{neuron_idx} = a{neuron_idx} & ~(a{neuron_idx} >> 31);\n"
...
def generate_second_layer_code(used_neurons, used_output_neurons):
    code = ""
    declarations = ""
    if used_output_neurons:
        declarations += "    int " + ", ".join([f"o{neuron_idx}" for neuron_idx in used_output_neurons]) + ";\n"
    # Now generate the code
...
def generate_output_code(used_output_neurons):
    code = ""
    # Define outputs array and indices array
    code += "    int outputs[] = {" + ", ".join([f"o{neuron_idx}" for neuron_idx in used_output_neurons]) + "};\n"
    code += "    int neuron_indices[] = {" + ", ".join([f"{neuron_idx}" for neuron_idx in used_output_neurons]) + "};\n"
    code += f"    int max_index = 0;\n"
    code += f"    int max_value = outputs[0];\n"
    code += f"    for (int idx = 1; idx < {len(used_output_neurons)}; idx++) {{\n"
    code += f"        if (outputs[idx] > max_value) {{\n"
    code += f"            max_value = outputs[idx];\n"
    code += f"            max_index = idx;\n"
    code += f"        }}\n"
    code += f"    }}\n"
    code += f"    return neuron_indices[max_index];\n"
    return code
...
def generate_predict_function():
    ...
    first_layer_code, first_layer_declarations = generate_first_layer_code(used_neurons, used_inputs)
    second_layer_code, second_layer_declarations = generate_second_layer_code(used_neurons, used_output_neurons)
    output_code = generate_output_code(used_output_neurons)

    # Generate the entire function code
    function_code = '''int relu(int x) {
    return x & ~(x >> 31);
}
int predict(int i[]) {
'''
    # Combine the variable declarations
    function_code += first_layer_declarations
    function_code += second_layer_declarations

    # Now add the code for the layers
...
with open('predict_function.c', 'w') as file:
